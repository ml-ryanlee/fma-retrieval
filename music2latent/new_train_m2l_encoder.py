...
def train_one_epoch(model: Encoder, train_loader: DataLoader, optimizer, loss_fn, device, gradient_accumulation_steps: int, scaler: GradScaler):
    model.train()
    total_loss = 0
...
def train_model(model, train_loader, val_loader, num_epochs, learning_rate, device, gradient_accumulation_steps, checkpoint_path='checkpoints'):
    ...
    for epoch in range(num_epochs):
        ...
        # Train
        train_loss = train_one_epoch(model, train_loader, optimizer, loss_fn, device, gradient_accumulation_steps, scaler)
