...
def train_one_epoch(model: LinearProbe, train_loader: DataLoader, optimizer, loss_fn, device):
    model.train()
    total_loss = 0
    ...
    for batch in tqdm(train_loader, desc='Training'):
        ...
        # Forward pass
        orig_emb = model({"waveform": original}, device=device)
        trans_emb = model({"waveform": transformed}, device=device)
...
def validate(model: LinearProbe, val_loader: DataLoader, loss_fn, device):
    model.eval()
    total_loss = 0
